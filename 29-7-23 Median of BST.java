...
class Tree
{
    public static float findMedian(Node root)
    {
       ...
       if(ar.size()%2!=0)
       {
           f=(float)ar.get(ar.size()/2);
       }
       else 
       {
           f=(float)(ar.get(ar.size()/2)+ar.get(ar.size()/2-1))/2;
       }
       return f;
    }
    public static ArrayList<Integer>trv(ArrayList<Integer>ar,Node root)
    {
        if(root==null)
        {
            return ar;
